...
import static org.junit.jupiter.api.Assertions.*;
    ...
    /**
     *  1. Admin selecteaza sala → specificatiile initiale sunt accesate din BD
     *  2. Decizia "modifici specificatiile?" → ramura NU → nicio modificare
     *  3. Decizia "modifici specificatiile?" → ramura DA → se aplica modificarile
     *  4. Decizia "cerere de confirmare a modificarilor" → ramura NU → modificarile nu se salveaza
     *  5. Decizia "cerere de confirmare a modificarilor" → ramura DA → modificarile se salveaza in BD
     *  6. Flux complet: modificare confirmata si persistata corect
     */
    class ModificareSpecificatiiSalaTest {
        ...
        @BeforeEach
        void setup() {
            admin = new Administrator(1, "Admin Popescu", "admin@biblioteca.ro",
                    "parolaAdmin", "IT", 1);
        ...
        void testSelectareSala_SpecificatiiInitialeAccesibile() {
            ...
            assertEquals(NUME_INITIAL, sala.getDenumire(),
                    "Denumirea initiala trebuie sa fie accesibila.");
            assertEquals(CAPACITATE_INITIALA, sala.getCapacitate(),
                    "Capacitatea initiala trebuie sa fie accesibila.");
            assertTrue(sala.isDisponibilitate(),
                    "Disponibilitatea initiala trebuie sa fie true.");
        }
        ...
        void testDecizie_NuModifica_SpecificatiiNeschimbate() {
            ...
            assertEquals(NUME_INITIAL, sala.getDenumire(),
                    "Denumirea nu trebuie modificata daca adminul alege NU.");
            assertEquals(CAPACITATE_INITIALA, sala.getCapacitate(),
                    "Capacitatea nu trebuie modificata daca adminul alege NU.");
        }
        ...
        void testDecizie_DaModifica_SpecificatiiActualizate() {
            ...
            assertEquals(numeNou, sala.getDenumire(),
                    "Denumirea salii trebuie actualizata.");
            assertEquals(capacitateNoua, sala.getCapacitate(),
                    "Capacitatea salii trebuie actualizata.");
        }

        // Testul 4: Cerere de confirmare → ramura NU
        // Adminul modifica in memorie dar NU confirma salvarea → BD ramane
        // neschimbata (simulat prin restaurarea valorilor initiale).
        ...
        void testConfirmare_Nu_ModificarileNuSePersista() {
            ...
            assertEquals(NUME_INITIAL, sala.getDenumire(),
                    "Fara confirmare, denumirea din BD trebuie sa ramana neschimbata.");
            assertEquals(CAPACITATE_INITIALA, sala.getCapacitate(),
                    "Fara confirmare, capacitatea din BD trebuie sa ramana neschimbata.");
        }
        ...
        @Test
        void testConfirmare_Da_ModificarileSePersista() {
            ...
            // Verificam ca persistenta a avut loc.
            assertEquals(numeNou, sala.getDenumire(),
                    "Dupa confirmare (DA), denumirea trebuie salvata.");
            assertEquals(capacitateNoua, sala.getCapacitate(),
                    "Dupa confirmare (DA), capacitatea trebuie salvata.");
        }
        ...
        void testFluxComplet_ModificareConfirmata() {
            ...
            assertEquals(numeNou, sala.getDenumire(),
                    "Denumirea finala trebuie sa fie cea modificata si confirmata.");
            assertEquals(capacitateNoua, sala.getCapacitate(),
                    "Capacitatea finala trebuie sa fie cea modificata si confirmata.");

            // Pas 5: Sala ramane in lista adminului
            assertTrue(admin.getSaliGestionate().contains(sala),
                    "Sala modificata trebuie sa ramana gestionata de admin.");
        ...
        void testModificare_Disponibilitate_SalaIndisponibila() {
            ...
            assertFalse(sala.isDisponibilitate(),
                    "Sala trebuie marcata INDISPONIBILA dupa modificare.");

            // Adminul restaureaza disponibilitatea
            admin.gestioneazaDisponibilitate(sala, true);

            assertTrue(sala.isDisponibilitate(),
                    "Sala trebuie marcata DISPONIBILA dupa restaurare.");
        }
    }
